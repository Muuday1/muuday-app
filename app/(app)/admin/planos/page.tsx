...
export default function AdminPlanosPage() {
  const [plans, setPlans] = useState<PlanConfigMap | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      setErrorMessage('')
      const response = await fetch('/api/admin/plan-config', { credentials: 'include' })
      const payload = await response.json().catch(() => null)
      if (!alive) return
      if (!response.ok || !payload?.ok || !payload?.plans) {
        setErrorMessage(payload?.error || 'Não foi possível carregar os planos.')
        setPlans(null)
        setLoading(false)
        return
      }
      setPlans(payload.plans as PlanConfigMap)
      setLoading(false)
    }
    load()
    return () => {
  ...
  const tierConfig = useMemo(() => (plans ? plans[activeTier] : null), [activeTier, plans])
    ...
    return (
      <div className="mx-auto max-w-4xl p-6 md:p-8">
        <p className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          {errorMessage || 'Sem dados para exibir.'}
        </p>
      </div>
    )
  }
