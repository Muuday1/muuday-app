...
function statusClasses(passed: boolean) {
  return passed ? 'border-green-200 bg-green-50 text-green-800' : 'border-amber-200 bg-amber-50 text-amber-800'
}
...
export default async function OnboardingProfissionalPage({
  ...
  return (
    <div className="mx-auto max-w-5xl p-6 md:p-8">
      <div className="mb-8">
        <h1 className="font-display text-3xl font-bold text-neutral-900">Onboarding profissional C1-C10</h1>
        <p className="mt-1 text-sm text-neutral-500">
          Checklist machine-checkable da Wave 2 para submissão, go-live, primeiro booking e payout.
        </p>
      </div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
        {Object.values(evaluation.gates).map(gate => (
          <div key={gate.id} className={`rounded-2xl border p-4 ${statusClasses(gate.passed)}`}>
            ...
            <p className="mt-1 text-xs">{gate.passed ? 'Aprovado' : gate.blockers[0]?.description || 'Bloqueado'}</p>
          </div>
        ))}
      </div>

      <div className="mb-6 rounded-2xl border border-neutral-100 bg-white p-5">
        <h2 className="mb-3 font-display text-xl font-semibold text-neutral-900">Estágios</h2>
      ...
      <div className="rounded-2xl border border-neutral-100 bg-white p-5">
        <h2 className="mb-3 font-display text-xl font-semibold text-neutral-900">Matriz de requisitos (C10)</h2>
            ...
            <tbody>
              {evaluation.matrix.map(row => (
                <tr key={row.field} className="border-t border-neutral-100">
                  ...
                  <td className="px-3 py-2">{row.required_for_first_booking_acceptance ? 'Sim' : '-'}</td>
                  <td className="px-3 py-2">{row.required_for_payout ? 'Sim' : '-'}</td>
                  <td className="px-3 py-2">
                    <span className={`rounded-full px-2 py-0.5 ${row.met ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}>
                      {row.met ? 'OK' : 'Pendente'}
                    </span>
                  </td>
                </tr>
      ...
      <div className="mt-6 rounded-2xl border border-neutral-100 bg-white p-5">
        ...
        <p className="mb-4 text-xs text-neutral-500">Envio para revisão usa gate machine-checkable (C2-C6).</p>
