...
| **MEDIUM-1** `createAdminClient()` migration complete | Replaced `provider_payment_id` with `stripe_payment_intent_id` across all payment code to align with production schema. Created migration 086 (RPC function) and 087 (fix RPC to target `stripe_payment_intent_id`). Migrated all three user-facing payment routes to use `supabase.rpc('update_payment_provider_id')` — `createAdminClient()` removed from user-facing payment routes. Fixed Stripe `confirmation_method` conflict with `automatic_payment_methods`. | `app/api/v1/payments/payment-intent/route.ts`, `app/api/stripe/payment-intent/route.ts`, `app/api/stripe/checkout-session/booking/route.ts`, `lib/stripe/webhook-handlers.ts`, `lib/stripe/capture.ts`, `lib/payments/refund/engine.ts`, `db/sql/migrations/086-rpc-update-payment-provider-id.sql`, `db/sql/migrations/087-fix-rpc-stripe-payment-intent-id.sql` |
