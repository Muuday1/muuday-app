...
| Professional onboarding | Done | Professional profile creation/editing and availability management are live. | Improve verification workflow depth and SLA automation. |
...
| CI and quality gates | Done | GitHub `ci.yml` runs typecheck/lint/build on push and PR. | Add automated test suites beyond build/lint gates. |
| Observability | In progress | Runbooks are in place; Checkly setup documented. | Activate and verify Checkly checks and alert channels. |
| Sentry/PostHog | Blocked | Not yet wired in code as production instrumentation baseline. | Add SDKs, event taxonomy, and alert ownership. |
...
1. `db/sql/schema/supabase-schema.sql` does not fully represent the latest migration state. Use ordered migrations as authoritative source.
