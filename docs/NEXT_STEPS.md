...
- **Acceptance:**
  - [x] `components/dashboard/OnboardingTrackerModal.tsx` (~~2,038~~ **1,009 lines**) → extracted presentational components (`StageSidebar`, `TrackerHeader`, `AdjustmentBanner`, `PlanFeatureBanner`) and domain hooks (`usePhotoState`, `useTermsState`, `useModalContext`, `useIdentityState`, `useServiceState`, `usePlanState`)
  ...
  - [x] `components/booking/BookingForm.tsx` (~~1,295~~ ~~752~~ **395 lines**) → extracted 9 presentational components + 6 custom hooks into `components/booking/booking-form/`
...
| C48 | **Cleanup Pass 34 — Next.js 16.2.4 `middleware.ts` → `proxy.ts` rename** — Renamed root `middleware.ts` → `proxy.ts` and changed exported function from `middleware` → `proxy` per Next.js 16.2.4 convention. Eliminates build deprecation warning. Updated `next.config.js` comment and `AGENTS.md` reference. TypeScript clean; 1052/1052 tests pass. | `proxy.ts` (new), `middleware.ts` (deleted), `next.config.js`, `AGENTS.md` |
| C49 | **Cleanup Pass 35 — Fix remaining `any` types in `app/` directory** — Replaced 7 remaining `: any` annotations across 3 pages with proper interfaces. Exported `CaseItem` from `CaseQueueClient.tsx` and used it in `admin/casos/page.tsx` (5 sites) and `disputas/page.tsx` (1 site). Added local `BookingOption` interface in `disputas/nova/page.tsx` for dropdown items (1 site). All `app/` pages now use explicit types. TypeScript clean; 1052/1052 tests pass. | `app/(app)/admin/casos/page.tsx`, `app/(app)/disputas/page.tsx`, `app/(app)/disputas/nova/page.tsx`, `components/admin/CaseQueueClient.tsx` |
| C50 | **Cleanup Pass 36 — Error page coverage + report staleness fixes** — Added `app/(app)/not-found.tsx` to provide contextual 404 handling for all `(app)` dynamic routes that call `notFound()`. Updated CODE_REVIEW_REPORT.md post-audit note to correct 4 stale entries: HIGH-2 `force-dynamic` (fixed in Pass 18), MEDIUM-1 `middleware.ts` (includes Pass 34 proxy rename), `any` in app/ (includes Pass 35), remaining open issues list. TypeScript clean; 1052/1052 tests pass. | `app/(app)/not-found.tsx`, `CODE_REVIEW_REPORT.md` |
| C51 | **Cleanup Pass 37 — Mobile critical bug + accessibility** — Fixed critical React bug in `mobile/app/booking/[id].tsx`: `setState()` inside `useMemo()` changed to `useEffect()` (could cause infinite loops). Added missing `useEffect` import. Added `accessibilityLabel` to 3 React Native `<Image>` components in `mobile/app/professional/[id].tsx` and `mobile/components/professional/ProfessionalCard.tsx`. TypeScript clean; 1052/1052 tests pass. | `mobile/app/booking/[id].tsx`, `mobile/app/professional/[id].tsx`, `mobile/components/professional/ProfessionalCard.tsx` |
| C52 | **Cleanup Pass 38 — Admin page type safety + `as any` removal** — Fixed root cause of type mismatches in 6 files: `getCaseById` service+action now select and return `assigned_to`, `priority`, `sla_deadline`, `summary` — removed 4 `as any` casts from `admin/casos/[caseId]/page.tsx`. `admin/avaliacoes/page.tsx` replaced `as any` with validated literal unions. `admin/finance/treasury/page.tsx` defined `TreasuryData` interface, removed 2 `as` casts. `admin/revisao/[professionalId]/page.tsx` fixed `social_links` type. TypeScript clean; 1052/1052 tests pass. | `lib/disputes/dispute-service.ts`, `lib/actions/disputes.ts`, `app/(app)/admin/casos/[caseId]/page.tsx`, `app/(app)/admin/avaliacoes/page.tsx`, `app/(app)/admin/finance/treasury/page.tsx`, `app/(app)/admin/revisao/[professionalId]/page.tsx` |
| C53 | **Cleanup Pass 39 — OnboardingTrackerModal hook extraction (P3.1 God File Refactoring)** — Extracted 7 domain hooks from the 2,038-line modal: `usePhotoState` (photo upload/crop/drag), `useTermsState` (terms acceptance flow), `useModalContext` (~300-line mega useEffect for async data hydration), `useIdentityState` (identity fields + qualifications), `useServiceState` (service CRUD form), `usePlanState` (tier selection + Stripe checkout), `useSaveSection` (generic API save wrapper with evaluation refresh + auto-advance). Also extracted 4 presentational components: `StageSidebar`, `TrackerHeader`, `AdjustmentBanner`, `PlanFeatureBanner`. Modal reduced to ~1,009 lines (-50%). Fixed Vercel build failure: changed `STRIPE_SECRET_KEY` validation `throw` → `console.warn` (CI=true during builds). TypeScript clean after each phase. | `components/dashboard/OnboardingTrackerModal.tsx`, `components/dashboard/onboarding-tracker/hooks/*`, `components/dashboard/onboarding-tracker/components/*`, `lib/config/env.ts` |
| C54 | **Cleanup Pass 40 — BookingForm.tsx presentational component extraction (P3.1 God File Refactoring)** — Extracted 9 presentational components from the 1,295-line booking form: `BookingSuccessRedirect`, `SelectedServiceCard`, `BookingTypeSelector`, `RecurringConfigPanel`, `TimezoneToggle`, `CalendarGrid`, `TimeSlotsGrid`, `SessionPurposeInput`, `BatchPanel`, `BookingSummarySidebar`. Extracted shared types to `booking-form/types.ts`. Main component reduced from 1,295 → 752 lines (-42%). TypeScript clean. | `components/booking/BookingForm.tsx`, `components/booking/booking-form/components/*`, `components/booking/booking-form/types.ts` |
| C55 | **Cleanup Pass 41 — BookingForm.tsx hook extraction (P3.1 God File Refactoring)** — Extracted 6 custom hooks from the 752-line booking form: `useCalendarSlots` (calendar state, slot generation, date navigation, prefill), `useRecurringBooking` (recurrence config, conflict detection, duration validation), `useBatchBooking` (batch date management, preview generation), `useBookingPricing` (price calculations, timezone conversions), `useBookingSubmission` (form submission, API v1/server-action branching, timeout handling, redirect), `useBookingAnalytics` (analytics tracking). Main component reduced from 752 → 395 lines (-70% total from 1,295). TypeScript clean. | `components/booking/BookingForm.tsx`, `components/booking/booking-form/hooks/*` |
| C56 | **Cleanup Pass 42 — ProfessionalAgendaPage.tsx component extraction (P3.1 God File Refactoring)** — Extracted 7 presentational components and 2 hooks from the 660-line agenda page: `AgendaHeader`, `AgendaStatsCards`, `UpcomingBookingsSection`, `OverviewCalendarSection`, `InboxHeader`, `InboxList`, `BlockTimeModal`, plus `useBlockTimeModal` and `useInboxItems`. Extracted shared types and 6 helper functions to dedicated files. Main component reduced from 660 → 134 lines (-80%). TypeScript clean. | `components/agenda/ProfessionalAgendaPage.tsx`, `components/agenda/professional-agenda/*` |
