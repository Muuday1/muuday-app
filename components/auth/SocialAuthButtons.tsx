...
const PROVIDERS = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path d="M17.05 20.28c-.98.95-2.05.88-3.08.4-1.09-.5-2.08-.48-3.24 0-1.44.62-2.2.44-3.06-.4C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.53 4.08zm-5.85-15.1c.07-2.04 1.76-3.79 3.75-3.94.29 2.32-1.93 4.48-3.75 3.94z" fill="#000"/>
      </svg>
    ),
  },
]
...
export default function SocialAuthButtons({
  ...
  async function handleSocialLogin(provider: Provider) {
    ...
    // Use server-side OAuth initiation to keep the URL on the custom domain
    // instead of showing the Supabase project URL in the browser.
    const oauthInitUrl = new URL('/api/auth/oauth', getClientAppBaseUrl())
    oauthInitUrl.searchParams.set('provider', provider)
    oauthInitUrl.searchParams.set('redirect_to', callbackUrl.toString())

    // Navigate to our proxy endpoint; it redirects to Supabase auth.
    window.location.href = oauthInitUrl.toString()
  }
