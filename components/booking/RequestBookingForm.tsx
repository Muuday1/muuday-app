...
export default function RequestBookingForm({
  ...
  if (result?.success) {
    return (
      <div className="mx-auto flex max-w-xl flex-col items-center rounded-lg border border-green-100 bg-white px-6 py-12 text-center">
        <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-green-50">
          <CheckCircle2 className="h-8 w-8 text-green-500" />
        </div>
        <h2 className="mb-2 text-2xl font-bold text-slate-900 font-display">Solicitação enviada</h2>
  ...
  return (
    <div className="mx-auto max-w-3xl p-6 md:p-8">
      ...
      <div className="rounded-lg border border-slate-200/80 bg-white p-6 md:p-7">
        <h1 className="mb-1 text-2xl font-bold text-slate-900 font-display">Solicitar horário</h1>
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
          <label className="space-y-1.5 text-sm">
            <span className="font-medium text-slate-700">Horário preferencial</span>
            <input
              type="datetime-local"
              value={preferredStartLocal}
              onChange={e => setPreferredStartLocal(e.target.value)}
              className="w-full rounded-md border border-slate-200 px-3 py-2.5 text-sm text-slate-700 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-[#9FE870]/40"
            />
          </label>
        ...
        <label className="space-y-1.5 text-sm">
          <span className="font-medium text-slate-700">Mensagem (opcional)</span>
          <textarea
            value={message}
            ...
            className="w-full resize-none rounded-md border border-slate-200 px-3 py-2.5 text-sm text-slate-700 placeholder-slate-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-[#9FE870]/40"
          />
          <span className="block text-right text-xs text-slate-400">{message.length}/1200</span>
        </label>
        ...
        <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-md bg-[#9FE870] px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-[#8ed85f] disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"
          >
