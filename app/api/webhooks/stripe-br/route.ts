...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Assinatura do webhook inválida.'
    const body: Record<string, string> = { error: 'Assinatura do webhook inválida.' }
  ...
  if (!admin) {
    return withCors(
      NextResponse.json(
        { error: 'Admin client não configurado para persistência de webhooks Stripe.' },
