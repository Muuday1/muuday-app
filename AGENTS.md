...
- **CSP with nonces**: Content-Security-Policy is set dynamically in `proxy.ts` with per-request nonces. `script-src` does not allow `'unsafe-inline'` — Next.js hydration scripts receive the nonce automatically.
