...
export function LoginForm({ title, subtitle, redirectTo, onSuccess, idPrefix = 'login' }: LoginFormProps) {
    ...
    <div>
      ...
      <form
        ...
        onSubmit={event => {
          ...
          // Minimal stub: rely on dedicated /login page for real auth.
          onSuccess?.()
        }}
      >
        <div>
          <label className="block text-xs font-medium text-neutral-500 mb-1.5" htmlFor={`${idPrefix}-email`}>
            Email
          </label>
          <input
            id={`${idPrefix}-email`}
            ...
            className="w-full rounded-xl border border-neutral-200 bg-white px-3 py-2.5 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/20 focus-visible:border-brand-500"
            required
          />
        </div>

        <div>
          <label className="block text-xs font-medium text-neutral-500 mb-1.5" htmlFor={`${idPrefix}-password`}>
            Senha
          </label>
          <input
        ...
        <button
          type="submit"
          className="w-full rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/30"
        >
          Entrar
        </button>
      </form>
