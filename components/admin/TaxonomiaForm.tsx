...
export function TaxonomiaForm({ initialData }: TaxonomiaFormProps) {
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto">
      <h1 className="font-display font-bold text-2xl text-slate-900 mb-2">Taxonomia</h1>
      <p className="text-sm text-slate-500 mb-6">Gerencie categorias, subcategorias e profissões/especialidades validadas.</p>

      <div className="flex gap-2 mb-6">
        <button
          onClick={() => setActiveTab('tree')}
          className={`px-4 py-2 rounded-md text-sm font-semibold transition-all flex items-center gap-2 ${activeTab === 'tree' ? 'bg-[#9FE870] text-white' : 'bg-white border border-slate-200 text-slate-600'}`}
        >
          <FolderTree className="w-4 h-4" /> Árvore
        </button>
        <button
          onClick={() => setActiveTab('services')}
          className={`px-4 py-2 rounded-md text-sm font-semibold transition-all flex items-center gap-2 ${activeTab === 'services' ? 'bg-[#9FE870] text-white' : 'bg-white border border-slate-200 text-slate-600'}`}
        >
          <Briefcase className="w-4 h-4" /> Serviços
        </button>
        <button
          onClick={() => setActiveTab('tags')}
          className={`px-4 py-2 rounded-md text-sm font-semibold transition-all flex items-center gap-2 ${activeTab === 'tags' ? 'bg-[#9FE870] text-white' : 'bg-white border border-slate-200 text-slate-600'}`}
        >
      ...
      {activeTab === 'tree' && (
        <div className="space-y-2">
          {categories.map(cat => {
            ...
            return (
              <div key={cat.id} className="bg-white rounded-lg border border-slate-200/80 overflow-hidden">
                <div
                ...
                {isExpanded && (
                  <div className="border-t border-slate-100/80 px-4 py-2 space-y-1">
                    {catSubs.map(sub => {
                      const subSpecs = specialties.filter(sp => sp.subcategory_id === sub.id)
                      const isSubExpanded = expandedSub === sub.id
                      return (
                        <div key={sub.id}>
                          <div className="flex items-center gap-2 py-2 pl-6 cursor-pointer hover:bg-slate-50/70 rounded-lg" onClick={() => setExpandedSub(isSubExpanded ? null : sub.id)}>
                            ...
                            <div className="pl-14 space-y-1 pb-2">
                              ...
                              {addItem?.type === 'specialty' && addItem.parentId === sub.id ? (
                                ...
                                <button
                                  ...
                                  className="flex items-center gap-1 text-xs text-[#9FE870] hover:text-[#3d6b1f] py-1"
                                >
                    ...
                    {addItem?.type === 'subcategory' && addItem.parentId === cat.id ? (
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-xs text-[#9FE870] hover:text-[#3d6b1f] py-2 pl-6"
                      >
      ...
      {activeTab === 'services' && (
        <div className="space-y-3">
          {subcategories.map(sub => {
            ...
            return (
              <div key={sub.id} className="bg-white rounded-lg border border-slate-200/80 p-4">
                <div className="mb-3 flex items-center justify-between gap-2">
                  <div>
                    <p className="text-sm font-semibold text-slate-900">{sub.name_pt}</p>
                    <p className="text-xs text-slate-500">{sub.slug}</p>
                  </div>
                  <span className="text-xs text-slate-500">{subServiceOptions.length} opções</span>
                </div>

                <div className="space-y-2">
                  {subServiceOptions.map(option => (
                    <div key={option.id} className="flex items-center gap-2 rounded-lg border border-slate-200/80 bg-slate-50/70 px-3 py-2">
                      {editItem?.id === option.id ? (
                        ...
                        <>
                          ...
                          <button
                            ...
                            className={`text-xs px-2 py-0.5 rounded-full ${option.is_active ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}
                          >
                  ...
                  {addItem?.type === 'service_option' && addItem.parentId === sub.slug ? (
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-xs text-[#3d6b1f] hover:text-[#3d6b1f]"
                    >
      ...
      {activeTab === 'tags' && (
        <div className="bg-white rounded-lg border border-slate-200/80 p-6">
          <h2 className="font-semibold text-slate-900 mb-4">Sugestões pendentes de revisão</h2>
          {tagSuggestions.length === 0 ? (
            <p className="text-sm text-slate-500">Nenhuma tag pendente.</p>
          ) : (
            <div className="space-y-3">
              {tagSuggestions.map(ts => (
                <div key={ts.id} className="flex items-center justify-between py-2 border-b border-slate-100/80 last:border-0">
                  <div>
                    <span className="text-sm font-medium text-slate-800">{ts.tag}</span>
                    <span className="text-xs text-slate-400 ml-2">{new Date(ts.created_at).toLocaleDateString('pt-BR')}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
